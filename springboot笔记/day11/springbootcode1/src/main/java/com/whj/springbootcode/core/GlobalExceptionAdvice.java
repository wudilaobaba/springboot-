...
//        }
    ...
    //===========================.前端POST方式值的时候，body体中的数据不是json格式.==============================
    @ExceptionHandler(HttpMessageNotReadableException.class)//处理异常 - 前端url参数校验异常
    @ResponseBody //该注解使得controller在抛出异常后，可接受到以下函数返回的值
    @ResponseStatus(code = HttpStatus.BAD_REQUEST) //400参数异常的http状态码
    public UnifyResponse handleConstraintException(HttpServletRequest req, HttpMessageNotReadableException e){
        String requestUrl = req.getRequestURI();//接口地址
        String method = req.getMethod(); //请求方式
        String request = method+" "+requestUrl;
        return new UnifyResponse(10003,"前端body体中的参数不符合json格式",request);
    }
}
